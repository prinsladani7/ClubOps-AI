...
describe("Production Health Probe API (/api/health)", () => {
  it("returns HTTP 200 with healthy status and database metrics", async () => {
    ...
    expect(data.database.entities.tasks).toBeGreaterThanOrEqual(0);
    expect(data.database.entities.judging_teams).toBeGreaterThanOrEqual(0);
    expect(data.database.entities.mentor_tickets).toBeGreaterThanOrEqual(0);
    expect(data.database.entities.sponsors).toBeGreaterThanOrEqual(0);
